...
int compute_levels (int64_t * level, int64_t nv, const int64_t * __restrict  bfs_tree, int64_t root)
{
	int err = 0;
...
int64_t Verify::verify_bfs_tree (int64_t *bfs_tree_in, int64_t max_bfsvtx, int64_t root,const packed_edge* IJ_in, int64_t nedge)
{
	...
	err = 0;
	nedge_traversed = 0;
	seen_edge = (int64_t*)Xalloc::xmalloc_large (2 * (nv) * sizeof (*seen_edge));
	level = &seen_edge[nv];
	...
	OMP("omp parallel shared(err)")
	{
		...
		for (k = 0; k < 2*nedge; k+=2)
		{
			...
			if (bfs_tree[i] >= 0 && bfs_tree[j] < 0)
				terr = -12;
			if (bfs_tree[j] >= 0 && bfs_tree[i] < 0)
				terr = -13;
			if (terr)
			{
				err = terr;
