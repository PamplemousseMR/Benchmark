#include "rmat.h"
	...
	struct packed_edge take_pe(struct packed_edge* p);
	void release_pe(struct packed_edge* p, struct packed_edge val);
#endif

void rmat_edge (struct packed_edge *out, int SCALE, double A, double B, double C, double D, const double *rn)
{
  size_t rni = 0;
  int64_t i = 0, j = 0;
...
void name(elt_type* A_in, int64_t nelem, mrg_state* __restrict st) \
{ \
...
void permute_vertex_labels (struct packed_edge* __restrict  IJ, int64_t nedge, int64_t max_nvtx, mrg_state* __restrict  st, int64_t * __restrict  newlabel)
{
	int64_t k;
	...
	OMP("omp for")
	for (k = 0; k < nedge; ++k)
		write_edge(&IJ[k], newlabel[get_v0_from_edge(&IJ[k])], newlabel[get_v1_from_edge(&IJ[k])]);
}

void permute_edgelist (struct packed_edge* __restrict  IJ, int64_t nedge, mrg_state* st)
{
...
void Rmat::rmat_edgelist(struct packed_edge *IJ_in, int64_t nedge, int SCALE, double A, double B, double C)
{
	struct packed_edge * __restrict  IJ = IJ_in;
	int64_t* __restrict  iwork;
	double D = 1.0 - (A + B + C);

	iwork = (int64_t*)Xalloc::xmalloc_large_ext ((size_t)((int64_t)(1L<<SCALE) * sizeof (*iwork)));

	OMP("omp parallel")
	{
		int64_t k;
		double* __restrict  Rlocal = (double*)malloc (NRAND(1) * sizeof (*Rlocal));
		mrg_state new_st = *(mrg_state*)Prng::getPrngState();
		...
		OMP("omp single")
			mrg_skip ((mrg_state*)Prng::getPrngState(), 1, NRAND(nedge), 0);
		OMP("omp barrier");
			new_st = *(mrg_state*)(mrg_state*)Prng::getPrngState();
		permute_vertex_labels (IJ, nedge, (int64_t)(1L<<SCALE), &new_st, iwork);
		OMP("omp single")
			mrg_skip ((mrg_state*)Prng::getPrngState(), 1, (uint_least64_t)(1L<<SCALE), 0);
		OMP("omp barrier");
			new_st = *(mrg_state*)(mrg_state*)Prng::getPrngState();
		permute_edgelist (IJ, nedge, &new_st);
	}
	mrg_skip ((mrg_state*)Prng::getPrngState(), 1, nedge, 0);
	Xalloc::xfree_large (iwork);
}
...
int64_t ake_i64 (volatile int64_t *p)
{
  int64_t out;
  do {
	OMP("omp critical (TAKE)") {
