...
void* Xalloc::xmalloc_large (size_t sz)
{
...
void Xalloc::xfree_large (void *p)
{
	#if defined(USE_MMAP_LARGE)||defined(USE_MMAP_LARGE_EXT)
	int k, found = 0;
	for (k = 0; k < n_large_alloc; ++k)
...
void* Xalloc::xmalloc_large_ext (size_t sz)
{
		...
		out = mmap (NULL, sz, PROT_READ|PROT_WRITE, MAP_SHARED|MAP_POPULATE|MAP_NOSYNC, fd, 0);
