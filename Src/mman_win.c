...
static int __map_mman_error(const DWORD err, const int val)
{
    if (err == 0)
        return 0;
    //TODO: implement
...
void* mmap(void* ptr, size_t len, int prot, int flags, int fildes, OffsetType off)
{
    HANDLE fm, h;
...
int msync(void *addr, size_t len, int val)
{
