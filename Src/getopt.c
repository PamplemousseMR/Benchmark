...
int parse_long_options(char * const *nargv, const char *options,const struct option *long_options, int *idx, int short_too)
{
...
int getopt_internal(int nargc, char * const *nargv, const char *options, const struct option *long_options, int *idx, int flags)
{
    ...
    if (posixly_correct == -1 || optreset != 0)
        posixly_correct = ((char*)_dupenv_s(&buf,&sz,"POSIXLY_CORRECT") != NULL);
    if (*options == '-')
        flags |= FLAG_ALLARGS;
