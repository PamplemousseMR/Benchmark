...
int createBuffer(cl_context* context, cl_mem_flags flag, size_t size, void* ptr, cl_mem* res)
{
	cl_int err;
...
int createProgram(cl_context* context,cl_uint i, const char* prog,const size_t* size, cl_program* program)
{
	cl_int err;
...
int buildProgram(cl_program* program, cl_context* context, cl_uint numDevices, cl_device_id* devices, const char* options, void(*notify)(cl_program, void*), void *data)
{
...
int createKernel(cl_program* program, const char* name, cl_kernel* kernel)
{
	cl_int err;
...
int enqueueWriteBuffer(cl_command_queue* command, cl_mem* mem, cl_bool blocking, size_t offset, size_t cb, const void *ptr, cl_uint num_events, const cl_event* events, cl_event* event)
{
...
int setKernelArg(cl_kernel* kernel, cl_uint num, size_t size, void *data)
{
...
int enqueueNDRangeKernel(cl_command_queue* command, cl_kernel* kernel, cl_uint work_dim, const size_t* work_offset, const size_t* global_work_size, const size_t* local_work_size, cl_uint num_event, const cl_event* list, cl_event* event)
{
...
int finish(cl_command_queue* command)
{
...
int releaseMemObject(cl_mem* mem)
{
...
int releaseProgram(cl_program* program)
{
...
int getPlatformIDs(cl_uint entries, cl_platform_id* platform, cl_uint* num_platforms)
{
...
int getDeviceIDs(cl_platform_id* platform,cl_device_type type,cl_uint entries,cl_device_id* devices, cl_uint* num_device)
{
...
int createContext(const cl_context_properties* properties, cl_uint num_devices,const cl_device_id* devices,void(*pfn_notify)(const char*, const void*, size_t, void*), void* user_data, cl_context* context)
{
	cl_int err;
...
int createCommandQueue(cl_context* context, cl_device_id* device,cl_command_queue_properties properties, cl_command_queue* command)
{
	cl_int err;
...
unsigned int getMaxWorkItem(cl_device_id* device)
{
	unsigned int i;
	cl_int err;
	cl_uint dim = 0;
...
size_t getMaxItemByGroup(cl_device_id* device)
{
	size_t si = 0;
...
int releaseCommandQueue(cl_command_queue* command)
{
...
int releaseContext(cl_context* context)
{
